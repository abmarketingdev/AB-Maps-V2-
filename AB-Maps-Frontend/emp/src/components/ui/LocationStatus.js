...
const LocationStatus = ({ locationService, permissionStatus: propPermissionStatus, onRequestPermission, isRequestingLocation = false }) => {
  ...
  const handleRequestPermission = async () => {
    // TESTING: Location permission request disabled for mobile testing
    console.log('Location permission request disabled for testing');
    // if (locationService) {
    //   try {
          ...
          <div className="permission-request">
            {!isRequestingLocation ? (
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <span style={{ color: '#1976d2', fontSize: '14px' }}>Requesting permission...</span>
              </div>
            )}
          </div>
        )}
