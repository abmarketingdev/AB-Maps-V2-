...
export default function ClusterDOMLayer({ items, color, onClick, pane = 'clusterPane', beforeClusterClick }) {
  ...
  // VALIDATE: Filter out clusters with invalid coordinates
  const validClusters = items.filter(c => {
    if (!c || typeof c.lat !== 'number' || typeof c.lng !== 'number' ||
        isNaN(c.lat) || isNaN(c.lng)) {
      console.warn('ClusterDOMLayer: Skipping invalid cluster:', c);
      return false;
    }
    return true;
  });
  ...
  return validClusters.map((c) => (
    <Marker
      ...
      eventHandlers={{
        ...
        click: (e) => {
          e?.originalEvent?.stopPropagation?.();
          if (typeof onClick === 'function') onClick(c);
        }
      }}
    />
  ));
