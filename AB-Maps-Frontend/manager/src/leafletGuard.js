// leafletGuard.js - Prevents Leaflet crashes from invalid latlng objects
...
if (!L.Map.prototype.__latlngGuarded) {
  ...
  // Patch the projection method that's causing the crash (with delayed initialization)
  ...
  L.Map.prototype._fireDOMEvent = function (e, type) {
    ...
    try {
      const result = __origFireDOMEvent.call(this, e, type);
  ...
  L.Map.prototype.latLngToContainerPoint = function(latlng) {
    try {
      if (!latlng || typeof latlng.lat !== 'number' || typeof latlng.lng !== 'number') {
        debugLog('🚨 [LeafletGuard] Invalid latlng in latLngToContainerPoint:', latlng);
        errorTracker.addInvalidLatLng(latlng, errorTracker.getStackTrace(), 'latLngToContainerPoint');
        // Return a safe default point
        return L.point(0, 0);
      }
      return __origLatLngToContainerPoint.call(this, latlng);
    } catch (error) {
