...
import { isNRCCampaignAsync, openNRCUrlForApartment } from '../../utils/nrcUrlHelper';
import { isNeiSubcategory } from '../../constants/neiSubcategory';
import './ApartmentListDrawer.css';

/**
 * ApartmentListDrawer Component
 * 
...
const ApartmentListDrawer = ({
  ...
  // NRC campaign state (pre-fetched for synchronous check in click handler)
  const [isNRCCampaignState, setIsNRCCampaignState] = useState(false);
  
  // Pre-fetch NRC campaign status when drawer opens
  useEffect(() => {
    if (isOpen) {
      const checkNRCCampaign = async () => {
        const isNRC = await isNRCCampaignAsync();
        console.log('[ApartmentListDrawer] Pre-fetched NRC campaign status:', isNRC);
   ...
   * No notes-only PATCH from this drawer.
   */
  const handleStatusChange = async (apartmentId, newStatus, notes = null, apartmentData = null, neiSubcategory = undefined) => {
    ...
    if (newStatus === 'ja' && apartment && isNRCCampaignState) {
