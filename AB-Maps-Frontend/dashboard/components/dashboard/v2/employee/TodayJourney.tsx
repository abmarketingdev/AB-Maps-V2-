  ...
  return (
    <div className="rounded-2xl border border-ab-line bg-ab-elevated backdrop-blur-xl p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-[13px] font-semibold text-ab-fg-2">Dagens runde</h3>
        <div className="flex items-center gap-3">
          {(Object.keys(OUTCOME_META) as Outcome[]).map((o) => (
            <span key={o} className="flex items-center gap-1.5 text-[11px] text-ab-fg-3">
        ...
        <div className="relative flex items-center gap-2 overflow-x-auto pb-2 scrollbar-thin">
          {events.map((e, i) => {
            ...
            return (
              ...
              >
                <span
                  className="rounded-full ring-2 ring-ab-base"
                  style={{
                ...
                />
                <span className="mt-1.5 text-[9px] font-mono text-ab-fg-4 opacity-0 group-hover/bead:opacity-100 transition-opacity absolute top-full whitespace-nowrap">
                  {e.time}
                </span>
              </motion.div>
            )
      ...
      {/* tally */}
      <div className="mt-4 flex items-center gap-4 border-t border-ab-line pt-3">
        {(Object.keys(OUTCOME_META) as Outcome[]).map((o) => (
          <div key={o} className="flex items-baseline gap-1.5">
            ...
            <span className="text-[11px] text-ab-fg-3">{OUTCOME_META[o].label}</span>
          </div>
        ))}
      </div>
    </div>
