...
import { Shield, Users as UsersIcon, Building2, CalendarRange } from "lucide-react"
  ...
  for (const chief of DEMO_CHIEFS) {
    for (const tm of chief.teams) {
      ...
      out.push({
        ...
        recruitedTotal: totalRec,  // demo-only pre-populated so winner badge paints on first load
...
async function fetchTeamsShallow(campaignId: string | undefined, period: string): Promise<TeamNode[]> {
  const list = await listTeams({ pageSize: 100, campaignId, period })
  return list.results.map((t, idx) => ({
    ...
    // Winning-team badge data — see SalgslederDashboard for rationale.
...
export function AdminDashboard() {
  ...
  // Winning team badge — moved INSIDE SalesChiefPanel 2026-08-10 so each chief
  // group gets its OWN "best team" badge (client ask: "for each salgssjef,
  // their best team should have badge"), instead of one global winner.
      ...
      <div className="relative max-w-[1600px] mx-auto">
        ...
        <div className="relative px-4 sm:px-6 py-6 sm:py-8 space-y-8">
          ...
          {/* SANNTID (TOP HALF only — KPIs + trend + mood). Campaign-per-
              recruits + live activity split off to the very bottom. */}
          <div>
          ...
          {/* Today's leaderboards — 2-col grid (doors + recruits) between
              SANNTID and Salgssjefer hierarchy. */}
          ...
          {/* LØNN section hidden for now (salary feature not ready to show).
              Client hasn't asked for admin-scope LØNN visibility yet — kept
              commented to match prod. Un-comment when client explicitly asks. */}
          ...
          {/* Rekrutterte per kampanje + Live aktivitet — moved to the very
              bottom per client ask 2026-08-09. */}
          <div>
