...
import { Pencil, Target, FolderOpen, Users, Copy, CheckCircle2, Calendar, Search, X } from "lucide-react"
...
export function MalInnstillinger() {
  ...
  // Free-text search — filters teams (by name / chief name / campaign name)
  // AND campaigns (by name). 2026-08-09 — with 20+ teams the vertical scroll
  // was painful; typing a chief or campaign name jumps you straight there.
  ...
  // Apply free-text search FIRST — filters both teams (by team name, chief
  // name, or campaign name) and campaigns (by campaign name). Empty search
  // = no-op, everything passes through. Case-insensitive substring match.
  const q = search.trim().toLowerCase()
  const teamsFiltered = useMemo(() => {
    if (!q) return teams
    return teams.filter((t) => {
      const name = t.name?.toLowerCase() ?? ""
      const chief = t.sales_chief?.name?.toLowerCase() ?? ""
      const campaign = t.campaign?.name?.toLowerCase() ?? ""
      return name.includes(q) || chief.includes(q) || campaign.includes(q)
    })
  }, [teams, q])

  const visibleCampaigns = useMemo(() => {
    const visibleIds = new Set(teamsFiltered.map((t) => t.campaign?.id).filter((x): x is string => Boolean(x)))
    let scoped = visibleIds.size === 0 ? [] : campaigns.filter((c) => visibleIds.has(c.id))
    // Search also matches campaign names directly — so typing "NRC" surfaces
    // the NRC campaign card even if none of its teams matched by another field.
    if (q) {
      const byName = campaigns.filter((c) => c.name?.toLowerCase().includes(q))
      const merged = new Map<string, CampaignVM>()
      for (const c of [...scoped, ...byName]) merged.set(c.id, c)
      scoped = Array.from(merged.values())
    }
    return scoped
  }, [teamsFiltered, campaigns, q])

  // Group teams by sales chief (uses teamsFiltered so search collapses empty
  // chief groups naturally — no chief header appears if none of their teams
  // matched the search).
  ...
  const matchCount = q ? teamsFiltered.length + visibleCampaigns.length : 0
    ...
    <div className="min-h-screen bg-ab-base">
      ...
      <div className="relative max-w-[1400px] mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-8">
        ...
        >
          ...
          {/* Search — team name, sales chief name, or campaign name.
              Client ask 2026-08-09 — 28 teams was too much to scroll. */}
          <div className="mt-4 flex flex-col sm:flex-row sm:items-center gap-3">
            <div className="relative flex-1 min-w-0">
              <Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-ab-fg-4" aria-hidden />
              <input
                type="text"
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder={t("Søk etter team, salgssjef eller prosjekt…")}
                aria-label={t("Søk")}
                className="w-full rounded-xl border border-ab-line bg-white/[0.03] pl-9 pr-9 py-2 text-sm text-ab-fg placeholder:text-ab-fg-4 focus:border-aurora-amber/50 focus:outline-none"
              />
              {search && (
                <button
                  type="button"
                  onClick={() => setSearch("")}
                  className="absolute right-2 top-1/2 -translate-y-1/2 inline-flex h-6 w-6 items-center justify-center rounded-md text-ab-fg-4 hover:bg-white/[0.06] hover:text-ab-fg"
                  aria-label={t("Tøm søk")}
                >
            ...
            {q && (
              <p className="text-[11px] text-ab-fg-3 shrink-0 sm:pl-2 sm:border-l sm:border-ab-line-1">
                <span className="font-mono text-ab-fg tabular-nums">{teamsFiltered.length}</span>{" "}
                {t("team")} ·{" "}
                <span className="font-mono text-ab-fg tabular-nums">{visibleCampaigns.length}</span>{" "}
                {t("prosjekt matcher")} "{search}"
              </p>
            )}
          </div>
        ...
        <div className="space-y-8">
          ...
          {!loading && teams.length > 0 && q && teamsFiltered.length === 0 && (
            <div className="rounded-2xl border border-dashed border-ab-line-1 bg-white/[0.02] px-6 py-8 text-center text-xs text-ab-fg-3">
              Ingen team matcher "<span className="text-ab-fg">{search}</span>".
              <button
                type="button"
                onClick={() => setSearch("")}
                className="ml-2 inline-flex items-center gap-1 rounded-full border border-ab-line px-2 py-0.5 text-[10px] font-semibold text-ab-fg-3 hover:border-aurora-amber/40 hover:text-ab-fg"
              >
                {t("Tøm søk")}
              </button>
            </div>
