...
//
// WeekPicker (2026-08-06) lets the user pick any ISO week within the selected
// month. Cards always render — GoalCard renders "?" for any missing number so
// leaders see honestly what's known vs. unset.
// ─────────────────────────────────────────────────────────────────────────────
...
export function PromoterMalRow({ period, campaignId }: PromoterMalRowProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchMyGoals({ period: activePeriod, week: weekParam }).catch(() => null),
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 xl:grid-cols-4 gap-3">
          {/* Personal doors — no per-person doors goal exists (would need a
              PersonalGoal model), so no goal slot; card is a pure count. */}
          <GoalCard
            ...
            compact
          />
          {/* Personal recruits — "din andel" derived goal from team_goal/members */}
          <GoalCard
            label={t("Antall givere")}
            value={p.recruited_monthly}
            goal={p.recruited_goal_share_monthly ?? null}
            accent="#3461FF"
          ...
          <GoalCard
            ...
            goal={tg.recruited_goal > 0 ? tg.recruited_goal : null}
            accent="#0E9384"
          ...
          <GoalCard
            ...
            goal={cg.recruited_goal > 0 ? cg.recruited_goal : null}
            accent="#F59E0B"
      ...
      {/* ═════════════════ MÅL UKE ═════════════════
          WeekPicker lets the promoter pick any ISO week in the selected month.
          Cards always render — GoalCard fills "?" for missing numbers. */}
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 xl:grid-cols-4 gap-3">
          {/* Personal doors weekly — no per-week doors actual computed yet */}
          <GoalCard
            label={t("Dører banket")}
            value={null}
            goal={0}
          ...
          <GoalCard
            ...
            goal={p.recruited_goal_share_weekly ?? null}
            accent="#3461FF"
            icon={<UserPlus className="h-3.5 w-3.5" />}
          ...
          <GoalCard
            ...
            goal={tg.recruited_weekly_goal ?? null}
            accent="#0E9384"
            icon={<Users className="h-3.5 w-3.5" />}
          ...
          <GoalCard
            ...
            goal={cg.recruited_weekly_goal ?? null}
            accent="#F59E0B"
            icon={<FolderOpen className="h-3.5 w-3.5" />}
