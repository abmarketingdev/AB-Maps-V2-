...
export default function SectionEditorPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (Number.isNaN(numId)) {
        setError("Ugyldig seksjons-id");
        return;
      }

      const s = await admin.getAdminSection(numId);
      setSection(s);
      setForm({
  ...
  }, [id, isNew, router, fetchCampaigns]);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setSavedAt(new Date());
      router.replace(`/admin-dashboard-learning/sections/${created.id}`);
    } catch (e) {
      console.error(e);
  ...
  const currentCampaignName =
    grouped?.campaigns?.find(
      (c) => String(c.id) === String(form.campaign ?? "null")
    )?.name ?? (form.campaign ? campaigns.find((c) => c.id === form.campaign)?.name : "Generell opplæring");
  ...
  if (error && !section && !isNew) {
    return (
      <ErrorState
        ...
        message={error}
        onGoHome={() => router.push("/admin-dashboard-learning/sections")}
      />
    );
  }

  return (
    <div className="flex min-h-[calc(100vh-5rem)] flex-col overflow-hidden bg-[#f7f7f7]">
      {/* Topp: Brødsmule, Sist lagret, Knapper */}
      <div className="flex shrink-0 items-center justify-between border-b border-[#E0E0E0] bg-white px-4 py-3">
        <div className="flex items-center gap-2 text-sm text-[#757575]">
          <Link href="/admin-dashboard-learning" className="hover:text-[#141414]">
            Hjem
          </Link>
          <span>/</span>
      ...
      <div className="flex min-h-0 flex-1">
        {/* Venstre: Innholdsstruktur – seksjoner per kampanje */}
        <aside className="flex w-[260px] shrink-0 flex-col border-r border-[#E0E0E0] bg-white">
          <div className="border-b border-[#E0E0E0] px-4 py-3">
            <h2 className="text-xs font-bold uppercase tracking-wider text-[#757575]">Innholdsstruktur</h2>
                  ...
                  <div key={cid}>
                    ...
                    {expanded &&
                      camp.sections.map((sec) => {
                        const isActive = !isNew && sec.id === parseInt(id, 10);
                        return (
                          <div
                            key={sec.id}
                            className={cn(
                          ...
                          >
                            <button
                              type="button"
                              onClick={() => router.push(`/admin-dashboard-learning/sections/${sec.id}`)}
            ...
            <Link
              ...
              className={cn(
                ...
                isNew
                  ...
                  : "border-[#E0E0E0] bg-white text-[#141414] hover:bg-[#f7f7f7]"
              )}
            >
          ...
          <div className="mx-auto max-w-2xl space-y-6">
            <div>
              ...
              <p className="mt-1 text-xs text-[#757575]">
                Generell opplæring for alle. Kampanjer kun for tildelte brukere.
              </p>
            </div>
            ...
            <div>
              ...
              <Input
                ...
                className="border-[#E0E0E0] text-lg font-semibold focus-visible:ring-emerald-500/50"
              />
            </div>
            ...
            <div>
              ...
              <Textarea
                ...
                rows={4}
                className="resize-y border-[#E0E0E0] focus-visible:ring-emerald-500/50"
              />
            </div>
        ...
        {/* Høyre: Forhåndsvisning – seksjonskort */}
        <aside className="hidden w-[360px] shrink-0 flex-col border-l border-[#E0E0E0] bg-white lg:flex">
          <div className="flex items-center justify-between border-b border-[#E0E0E0] px-4 py-3">
            <h3 className="text-xs font-bold uppercase tracking-wider text-[#757575]">Forhåndsvisning</h3>
            <span className="flex items-center gap-1.5 text-xs font-medium text-emerald-600">
              <span className="h-2 w-2 rounded-full bg-emerald-500" /> LIVE
            </span>
          </div>
          <div className="flex flex-1 flex-col items-center overflow-y-auto p-4">
            <div className="w-full max-w-[280px] overflow-hidden rounded-xl border border-[#E0E0E0] bg-white shadow-sm">
              <div className="flex items-center gap-3 p-4">
                ...
                <div className="min-w-0 flex-1">
                  <h4 className="truncate font-semibold text-[#141414]">{form.title || "Seksjonstittel"}</h4>
              ...
              <div className="border-t border-[#E0E0E0] px-4 py-3">
                <p className="line-clamp-3 text-sm text-[#757575]">
                  {form.description || "Beskrivelse vises her."}
                </p>
                <div className="mt-2">
                  <Badge show={form.is_active !== false}>Aktiv</Badge>
                </div>
              </div>
            </div>
            {!isNew && section && (
