...
// First-time password creation for a newly-invited user. Shares the reset-password token
// machinery (uid+token → confirmPasswordReset), but the copy welcomes rather than "resets".
            ...
            <motion.div key="invalid" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="text-center space-y-5">
              ...
              <div>
                ...
                <p className="text-[13px] text-ab-fg-2 mt-2 leading-relaxed">
                  Lenken for å sette passord er ikke lenger gyldig. Prøv å logge inn med brukernavnet ditt — du blir sendt hit på nytt med en fersk lenke.
                </p>
              </div>
          ...
          {phase === "success" && (
            <motion.div key="success" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="text-center space-y-5">
              ...
              <div>
                ...
                <h1 className="font-display text-[24px] font-semibold tracking-tight text-ab-fg">Passordet er satt</h1>
                <p className="text-[13px] text-ab-fg-2 mt-2 leading-relaxed">Du kan nå logge inn med brukernavnet og det nye passordet ditt. Sender deg til innlogging…</p>
              </div>
              <button onClick={() => router.push("/login")} className="ab-btn primary lg w-full justify-center cursor-pointer">
                Til innlogging
              </button>
          ...
          {(phase === "form" || phase === "submitting") && (
            <motion.div key="form" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
              ...
              <div className="text-center mt-4 mb-6">
                <div className="eyebrow">Velkommen til AB System</div>
                <h1 className="font-display text-[26px] font-semibold tracking-tight text-ab-fg mt-1">Sett ditt passord</h1>
                <p className="text-[13px] text-ab-fg-2 mt-1.5">Lag et sterkt passord du ikke bruker andre steder, så er brukeren din klar.</p>
              </div>
              <form onSubmit={submit} className="space-y-5">
                ...
                <button type="submit" disabled={phase === "submitting" || !pw || !confirm} className={cn("ab-btn primary lg w-full justify-center cursor-pointer", "disabled:opacity-50 disabled:cursor-not-allowed")}>
                  {phase === "submitting" ? <><Loader2 className="h-4 w-4 animate-spin" /> Lagrer…</> : "Sett passord"}
                </button>
