...
// New type for the sales page endpoint
...
/**
 * Fetch sales forms for a campaign
 */
export async function fetchSalesFormsByCampaign(campaign_id: string): Promise<SalesForm[]> {
  const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL;
  const token = await authService.getAccessToken();
  if (!token) throw new Error('Authentication required');
  const url = `${API_BASE_URL}/api/campaigns/campaign-forms/by_campaign/?campaign_id=${encodeURIComponent(campaign_id)}`;
  const response = await fetch(url, {
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json',
...
/**
 * Fetch sales data from the new sales page endpoint
 */
export async function fetchSalesPageData(campaign_id: string, employee_id: string): Promise<SalesPageData[]> {
  const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL;
  const token = await authService.getAccessToken();
  if (!token) throw new Error('Authentication required');
  
  const url = `${API_BASE_URL}/api/dashboard/sales-page/?campaign_id=${encodeURIComponent(campaign_id)}&employee_id=${encodeURIComponent(employee_id)}`;
  ...
  if (!response.ok) {
    throw new Error('Failed to fetch sales page data');
  }
